...
%% =============Kept unchanged for across each participant
    NOsnrsteps=1;
    noise_filename_def_range='input\T_m4dB_SSN.wav';
...
%% session 1: define range(9steps identification)
    ...
    def_range.indSNRs_k=[-4 -3]; % 默认SNR是tone-4phon-3;
    ...
    uiwait(msgbox('请注意您现在使用的definerange矩阵第二维度相同；要使用第二维度变化的正交矩阵，请选用“diti_defrange_par_EXPdata”文件。'))
    def_range.w_subj_stimsource=repmat({'input/diti_defrange_par_EXPdata_contin.mat','input/diti_defrange_par_EXPdata_contin.mat';...
...
%% session 2: practice(5steps identification + discrimination)
    ...
    practice.indSNRs_k=[-4 -3 -4 -3]; % 默认SNR是tone-4phon-3;
...
%% session3: identification_discrimination
    ...
    Id_Di.soundk=repmat({'soundint_k.mat','soundint_k.mat','soundint_k.mat','soundint_k.mat','soundint_k.mat','soundint_k.mat','soundint_k.mat','soundint_k.mat'},1,NOsnrsteps);

    Id_Di.w_subj_cond=repmat({'T_old_BEHAV','P_old_BEHAV','T_old_BEHAV','P_old_BEHAV','T_old_BEDIS','P_old_BEDIS','T_old_BEDIS','P_old_BEDIS'},1,NOsnrsteps);
    
    Id_Di.ses2_cond=cell(1,NOsnrsteps*8);
    Id_Di.noise_filename=cell(1,NOsnrsteps*8);
    Id_Di.ses2_cond{1,1}='T_CLEAR_ID';
    Id_Di.ses2_cond{1,2}='P_CLEAR_ID';
    Id_Di.ses2_cond{1,3}='T_NOISE_ID';
    Id_Di.ses2_cond{1,4}='P_NOISE_ID';
    Id_Di.ses2_cond{1,5}='T_CLEAR_DI';
    Id_Di.ses2_cond{1,6}='P_CLEAR_DI';
    Id_Di.ses2_cond{1,7}='T_NOISE_DI';
    Id_Di.ses2_cond{1,8}='P_NOISE_DI';
    
    Id_Di.noise_filename(1,1:8)=repmat({'input\T_m4dB_SSN.wav','input\P_m4dB_SSN.wav'},1,4);
    Id_Di.indSNRs_k=[-4 -3 -4 -3 -4 -3 -4 -3]; % 默认SNR是tone-4phon-3;
        
    Id_Di.filename=repmat({{'T_old_BEHAV';'tone_old'},{'P_old_BEHAV';'phon_old'},{'T_old_BEHAV';'tone_old'},{'P_old_BEHAV';'phon_old'},{'T_old_BEDIS';'tone_old'},{'P_old_BEDIS';'phon_old'},{'T_old_BEDIS';'tone_old'},{'P_old_BEDIS';'phon_old'}},1,NOsnrsteps);
                        
    Id_Di.Di_stimcluster=repmat({steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster,steps5_Di_stimcluster},1,NOsnrsteps);
    Id_Di.Di_xaxis=repmat({steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis,steps5_Di_xaxis},1,NOsnrsteps);
    
    Id_Di.w_subj_stimsource=repmat({ind_par_EXPdata_log_d;ind_xs_perc_struct_log_d},1,NOsnrsteps*8);

    Id_Di.stimsourcetype='_avg_';                
        
    Id_Di.w_cond_order=[1 2 3 4 5 6 7 8]; 
    %Id_Di.w_cond_order=1:NOsnrsteps*2;
    
    Id_Di.Zjudge_text=repmat({'/一声/','/d_/','/一声/','/d_/','/一声/','/d_/','/一声/','/d_/'},1,NOsnrsteps);
    
    Id_Di.Mjudge_text=repmat({'/二声/','/t_/','/二声/','/t_/','/二声/','/t_/','/二声/','/t_/'},1,NOsnrsteps);
    
    Id_Di.addnoise=[0,0,1,1,0,0,1,1];  

    Id_Di.measure_type=1*ones(1,NOsnrsteps*8); % 1=Constant; 2=Staircase;
    Id_Di.interact_type=2*ones(1,NOsnrsteps*8); % 1=Button; 2=Keyboard;
    Id_Di.staircase_id_dis=[2*ones(1,NOsnrsteps*4),3*ones(1,NOsnrsteps*4)]; % 1=Staircase; 2=Identification; 3=Discrimination
    
    Id_Di.staircase_stoprunstage=3*ones(1,NOsnrsteps*8);
    Id_Di.staircase_stoprunchange=4*ones(1,NOsnrsteps*8);
    
%     Id_Di.const_reptimes=[2 2 2 2 1 1 1 1];
    Id_Di.const_reptimes=[4 4 4 4 1 1 1 1];
    
    %=================================================================     
    DATA=lbsCP2_loadDATA();
    % check
    if size(DATA,2)>=par_code && ~isempty(DATA(par_code).par_info)
        disp(DATA(par_code));
        warndlg('该被试编号或被使用，本次初始化被试失败，请检查！');
    else
