...
% load sound parts:
ba55_burst=audioread('input/ba55_burst.wav');
ba55_vowel_pa55=audioread('input/ba55_vowel_pa55.wav');
pa55_VOT=audioread('input/pa55_VOT.wav');
pa55_vowel=audioread('input/pa55_vowel.wav');

votdur=length(pa55_VOT)/fs;
votint_len=votdur/nosteps_phon;

%di35_burst=audioread('di35_burst.wav');
ba35_vowel_pa35=audioread('input/ba35_vowel_pa35.wav');
%ti35_VOT=audioread('ti35_VOT.wav');
pa35_vowel=audioread('input/pa35_vowel.wav');
...
vowel_rms=mean([rms(ba55_vowel_pa55),rms(pa55_vowel),rms(ba35_vowel_pa35),rms(pa35_vowel)]);

%votint_len = (votint_len_tone1 + votint_len_tone2)/2;


di1_ti1_contin = bsliang_phoneme_ratedcontinuum_Rate(ba55_burst,pa55_VOT,ba55_vowel_pa55,pa55_vowel,1,xs_perc.phon_old,vowel_rms);
%sound(di1_ti1_contin_cat,fs);
% audiowrite('di1_ti1_contin_cat.wav',di1_ti1_contin_cat,fs);
% save di1_ti1_contin di1_ti1_contin

di2_ti2_contin = bsliang_phoneme_ratedcontinuum_Rate(ba55_burst,pa55_VOT,ba35_vowel_pa35,pa35_vowel,2,xs_perc.phon_old,vowel_rms);
...
for RMS_step=1:size(RMS_phon_tone_index,2)
    ...
    TP_temp=0.99*TP_temp/max(abs(TP_temp));
    TP_perc = xs_perc.phon_old(phon_step);
    [sound_asp,sound_t]=bsliang_connect_burst_vot_vowel_balanceRMS(ba55_burst,pa55_VOT,TP_perc,TP_temp,vowel_rms);
    all_morphed_stim_old{1,phon_step}{1,tone_step}=[sound_asp;sound_t];
