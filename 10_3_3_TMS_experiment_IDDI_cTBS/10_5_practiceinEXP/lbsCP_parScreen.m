...
for soundID=1:size(stimfile,2)
   ...
   PsychPortAudio('FillBuffer',soundBuffer{1,soundID}, [adjust_k(1)*stimfile{1,soundID}';adjust_k(2)*stimfile{1,soundID}']);
end
...
%         WaitSecs(1.4+(1.6-1.4)*rand(1,1)-rt); %BSLiang 20201025, 由于硬件的限制，TMS立刻开始下一个trial的刺激，所以要等够1.4s（1.4~1.6s）
        if staircase_id_dis==3
            pause(1.2); %由于硬件的限制，DIS要再等1.2s（即ITI=1.4+1.2=2.6s，2.6~2.8s jitter）
        end
        break;
    elseif keyCode(mKey)
        tEnd = GetSecs;
