...
% adjust_k=[0.6918 0.7161*1.0233];  % 2020年10月12日预实验

% 2020年10月12日，因为有TMS噪声干扰，要求78dB，k=sqrt(10^(78/10)/10^(X/10))
%  adjust_k=[0.5559*0.7499 0.7161*1.0839*0.9772*0.5821*0.9226];  % 2021年1月13日，用我自己的森海耳机
%  adjust_k=[0.6918*1.0839*0.2239 0.7161*1.0839*0.9772*0.2113*0.9441];  % 2020年10月12日预实验
 
 % 2021年3月18日，改为了cTBS，要求65dB，k=sqrt(10^(65/10)/10^(X/10))
 adjust_k=[0.0335 0.0355];
...
addnoise=1; %是否加入噪声？
SNR=-8; %原始噪声的SNR是-4，但是真正实验可能用到的最大SNR是-8，根据听力健康的标准，加入最大SNR时应当不超过LApeak82dB

[noise_bg,fs_noise_bg]=audioread('input\P_m4dB_SSN.wav');

signal0=par_EXPdata.P_old_BEHAV{1,1};
[~,noise_adjust_k]=bsliang_adjustnoiseSNR(signal0,noise_bg,SNR);

soundBuffer{1,end}=PsychPortAudio('OpenSlave',param,1,2);
PsychPortAudio('FillBuffer',soundBuffer{1,end}, [adjust_k(1)*noise_bg'/noise_adjust_k;adjust_k(2)*noise_bg'/noise_adjust_k]);
